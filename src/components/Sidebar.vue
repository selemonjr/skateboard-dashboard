<template>
  <div class="px-8 pt-2 -mt-10 divide-y divide-gray-800">
    <div v-for="group in sidebarMenu" :key="group.name" class="py-10">
      ...
      <ul class="flex flex-col pt-5 space-y-8">
        <li v-for="menu in group.children" :key="menu.name" class="flex items-center space-x-4 group">
          ...
          <a class="text-sm text-gray-500 group-hover:text-white group-hover:font-semibold" href="#">{{ menu.name }}</a>
        </li>
      </ul>
    </div>
  </div>
...
import PaperDownloadIcon from "../icons/paper-download.svg";
