<template>
  <div class="w-64 rounded-2.5xl overflow-hidden relative">
    ...
    <div class="absolute flex items-center justify-center rounded-full right-5 top-34 w-14 h-14">
      <img class="w-12 h-12 rounded-full" :src="content.avatar" alt="" />
    ...
    <div class="px-5 py-5 bg-gray-800">
      <div class="flex items-center">
        <span class="text-xs font-normal tracking-wider text-gray-400">{{ content.name }}</span>
        <span class="w-2 h-2 ml-2 rounded-full" :class="content.color === 'green' ? 'bg-green' : 'bg-orange'"/>
      </div>
